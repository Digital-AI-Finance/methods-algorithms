...
def extract_metadata(soup: BeautifulSoup) -> dict:
    ...
    metadata = {'title': 'Untitled', 'author': 'Methods and Algorithms'}
...
def split_frames_to_sections(soup: BeautifulSoup) -> list:
    ...
    for section in slides_container.find_all('section', class_='slide'):
        ...
        for frame in frames:
            ...
            # Add fragment class to li elements
            frame_html = re.sub(r'<li>', '<li class="fragment">', frame_html)
...
def convert_tex_to_revealjs(tex_file: Path, output_dir: Path) -> Path:
    ...
    try:
        ...
        if not sections:
            ...
            # Fall back to original Pandoc output
        ...
        print(f"  OK: {tex_file.name} -> {output_html.name} ({len(sections)} slides)")
        return output_html

    except Exception as e:
        print(f"  ERROR (Processing): {tex_file.name} - {e}")
        # Fall back to Pandoc output if processing fails
        if temp_html.exists():
            temp_html.rename(output_html)
            return output_html
        return None
...
def convert_all():
    ...
    # Copy theme CSS if not present
    theme_src = PROJECT_ROOT / "docs" / "slides" / "css" / "ml-theme.css"
    theme_dst = css_dir / "ml-theme.css"
    if theme_src.exists() and not theme_dst.exists():
        import shutil
