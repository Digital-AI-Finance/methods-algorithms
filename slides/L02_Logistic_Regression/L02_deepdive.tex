...
\begin{equation}
  \text{SE}(\hat{w}_j) = \sqrt{[\mathbf{H}^{-1}]_{jj}}
\end{equation}
...
\end{itemize}
\bottomnote{SE answers: ``if we repeated this study, how much would $\hat{w}$ vary?''}
\end{frame}
...
\begin{itemize}
  \item $H_0$: $w_j = 0$ (feature has no effect)
  \item $H_1$: $w_j \neq 0$ (feature matters)
\end{itemize}

\textbf{Wald Statistic (z-score):}
\begin{equation}
  z = \frac{\hat{w}_j}{\text{SE}(\hat{w}_j)} \;\;\xrightarrow{H_0}\;\; \mathcal{N}(0,1)
\end{equation}
...
\begin{equation}
  \hat{w}_j \pm 1.96 \times \text{SE}(\hat{w}_j)
\end{equation}

\textbf{95\% CI for Odds Ratio} (exponentiate both bounds):
\begin{equation}
  \exp\!\left(\hat{w}_j \pm 1.96 \times \text{SE}(\hat{w}_j)\right)
\end{equation}

\vspace{0.3em}
\textbf{Worked Example:} Income coefficient $\hat{w} = 0.5$, SE $= 0.1$
\begin{itemize}
  \item CI for $w$: $[0.304,\; 0.696]$
...
\item $\hat{w}_j \to \pm\infty$; algorithm fails to converge
\end{itemize}
