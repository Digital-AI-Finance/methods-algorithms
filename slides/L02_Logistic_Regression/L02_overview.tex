...
\begin{itemize}
  \item Linear regression predicts $\hat{y} = \mathbf{w}^\top \mathbf{x}$, which is unbounded
...
\end{itemize}
\begin{equation}
P(y=1|\mathbf{x}) = \sigma(z) = \frac{1}{1 + e^{-z}}, \quad z = b + \mathbf{w}^\top \mathbf{x}
\end{equation}
\begin{itemize}
...
\end{center}
\end{columns}
\bottomnote{Decision boundary: $b + \mathbf{w}^\top \mathbf{x} = 0$ -- a linear separator in feature space}
\end{frame}
...
\begin{itemize}
  ...
  \item Full likelihood: $L(\mathbf{w}) = \prod_{i=1}^{n} p_i^{y_i}(1-p_i)^{1-y_i}$
\end{itemize}
...
\end{itemize}
\bottomnote{Cross-entropy loss is convex in $\mathbf{w}$ -- optimization is well-behaved}
\end{frame}
...
\begin{itemize}
  \item The gradient of the log-likelihood with respect to $\mathbf{w}$:
\end{itemize}
\begin{equation}
\frac{\partial \ell}{\partial \mathbf{w}} = \sum_{i=1}^{n}(y_i - p_i)\,\mathbf{x}_i = \mathbf{X}^\top(\mathbf{y} - \mathbf{p})
\end{equation}
...
\begin{equation}
\log\frac{p}{1-p} = b + w_1 x_1 + \cdots + w_k x_k
\end{equation}

\vspace{0.3em}
\textbf{Coefficient Interpretation}
\begin{itemize}
  \item $w_j$: a one-unit increase in $x_j$ changes the \textbf{log-odds} by $w_j$
  \item $e^{w_j}$: the \textbf{odds ratio} -- multiplicative effect on the odds
