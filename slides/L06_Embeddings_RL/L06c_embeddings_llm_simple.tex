...
% Slide 13b: Polysemy Visualization
\begin{frame}{Can You See the Problem?}
\centering
\includegraphics[width=0.65\textwidth]{17_static_vs_contextual_embedding/chart.pdf}
\begin{compactlist}
\item Static embeddings place ``bank'' at ONE point between two clusters
\item Contextual embeddings (BERT) split it into two distinct meanings
\item This is why the evolution from Word2Vec to BERT matters
\end{compactlist}
\bottomnote{Static: one vector per word type. Contextual: one vector per word token (different in each sentence).}
\end{frame}
