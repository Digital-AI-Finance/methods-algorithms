...
\item \highlight{$\epsilon$-greedy}: with probability $\epsilon$ explore randomly, otherwise exploit the best known action
\end{compactlist}

\vspace{1mm}
\scriptsize Typical schedule: start with $\epsilon = 1.0$ (all exploration) and decay to $\epsilon = 0.01$ over training.
...
\node[font=\tiny\bfseries, mlpurple] at (2.0,2.0) {$\epsilon$-greedy policy};
\node[font=\tiny] at (2.0,1.5) {$P(\text{explore}) = \epsilon$};
\node[font=\tiny] at (2.0,1.0) {$P(\text{exploit}) = 1 - \epsilon$};
% Arrows
...
\item \highlight{Exploration}: $\epsilon$-greedy balances trying new actions (explore) with choosing the best known action (exploit). Start high, decay over time.
