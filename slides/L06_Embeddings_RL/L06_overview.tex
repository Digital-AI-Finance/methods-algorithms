...
\begin{frame}{Outline}
  \tableofcontents
\bottomnote{Two topics today: text embeddings and reinforcement learning}
\end{frame}
...
% Slide 8: Why Can't Machines Read Text?
\begin{frame}[t]{Why Can't Machines Read Text?}
...
% Slide 9: Word Embedding Space (CHART 01)
...
% Slide 10: How Do We Turn Words Into Numbers?
\begin{frame}[t]{How Do We Turn Words Into Numbers?}
...
% Slide 11: Cosine Similarity (MVF: Motivate-Visualize-Formalize)
\begin{frame}[t]{How Do We Measure Word Similarity?}
\textbf{Intuition:} Two vectors pointing in a similar direction encode similar meaning.

\vspace{0.3em}
\begin{center}
\begin{tikzpicture}[scale=1.2]
  \draw[->,thick,mlblue] (0,0) -- (3,4) node[right] {$\vec{u}=[3,4]$};
  \draw[->,thick,mlorange] (0,0) -- (4,3) node[right] {$\vec{v}=[4,3]$};
  \draw[mlgray,dashed] (0.6,0.8) arc[start angle=53.1,end angle=36.9,radius=1.0];
  \node[mlgray] at (1.3,1.1) {$\theta$};
\end{tikzpicture}
\end{center}
...
\textbf{Example:} $\frac{3 \times 4 + 4 \times 3}{5 \times 5} = \frac{24}{25} = 0.96$ \quad (very similar direction)
\bottomnote{Range: $[-1, 1]$. Cosine similarity = 1 means identical direction, 0 means unrelated, $-1$ means opposite}
\end{frame}

% Slide 12: Does 'Bank' Always Mean the Same Thing?
\begin{frame}[t]{Does `Bank' Always Mean the Same Thing?}
...
\end{itemize}
\bottomnote{Static: one meaning per word. Contextual: meaning adapts to context. Next: sequential decisions.}
\end{frame}
...
% Slide 14: How Does an Agent Learn from Rewards?
\begin{frame}[t]{How Does an Agent Learn from Rewards?}
\textbf{What is Q(s, a)?}
\begin{itemize}
...
% Slide NEW: RL Taxonomy Teaser
\begin{frame}[t]{Beyond Q-Learning: The RL Family}
\begin{compactlist}
  \item \textbf{Value-based} (today): Q-learning learns what each action is worth
  \item \textbf{Policy-based} (L06e): PPO learns the action selection directly
  \item \textbf{Actor-Critic} (deepdive): Combines both --- Actor picks, Critic evaluates
\end{compactlist}
\vspace{0.5em}
\begin{block}{Q-Learning Is Just the Beginning}
Today we cover the foundation. The deepdive adds DQN, Actor-Critic, and the full method taxonomy.
\end{block}
\bottomnote{All these methods build on the same agent-environment loop you learned today.}
\end{frame}

\section{Solution}

% Slide 15: Does the Agent Actually Improve? (CHART 05)
\begin{frame}[t]{Does the Agent Actually Improve?}
\begin{center}
...
% Slide 16: How Can Embeddings Detect Market Sentiment?
\begin{frame}[t]{How Can Embeddings Detect Market Sentiment?}
\textit{Returning to embeddings---}
...
% Slide 16b: FinBERT Sentiment Visualization
\begin{frame}[t]{What Does Sentiment Classification Look Like?}
\centering
\includegraphics[width=0.65\textwidth]{16_finbert_sentiment_bars/chart.pdf}
\vspace{2mm}
\begin{itemize}
\item Each headline compared to positive/negative sentiment anchors via cosine similarity
\item Green bars = positive similarity, red bars = negative similarity
\item FinBERT captures financial nuance (``hawkish'' = negative for equities)
\end{itemize}
\bottomnote{Cosine similarity measures alignment between embedding vectors: higher = more similar meaning}
\end{frame}
...
% Slide 18: Embeddings vs. RL: When Do You Use Which?
\begin{frame}[t]{Embeddings vs.\ RL: When Do You Use Which?}
\begin{center}
...
% Slide 20: What Are the Common Pitfalls?
\begin{frame}[t]{What Are the Common Pitfalls?}
\textbf{Embeddings}
\begin{itemize}
...
\begin{itemize}
\item Start simple: tabular Q-learning before more advanced methods
...
% Slide 21: Can You Build It Yourself?
\begin{frame}[t]{Can You Build It Yourself?}
...
\item Q-learning provides a practical algorithm for value-based control
\end{itemize}
\vspace{0.3em}
...
\begin{frame}[t]{The Complete ML Toolkit}
\begin{center}
\begin{tikzpicture}[scale=0.85]
  % Robot stick figure
  \node[rectangle, draw, rounded corners, minimum size=8mm, fill=mllavender4] (head) at (0,2.8) {\footnotesize AI};
  \draw[thick] (0,2.4) -- (0,1.4);
  \draw[thick] (0,2.0) -- (-0.6,1.5);
  \draw[thick] (0,2.0) -- (0.6,1.5);
  \draw[thick] (0,1.4) -- (-0.4,0.6);
  \draw[thick] (0,1.4) -- (0.4,0.6);

  % Toolbelt items
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (-3,1.5) {L01: Regression};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (-3,0.9) {L02: Classification};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (-3,0.3) {L03: Clustering};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (3,1.5) {L04: Ensembles};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (3,0.9) {L05: Dim.\ Reduction};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (3,0.3) {L06: Embeddings + RL};

  % Speech bubble
  \node[draw, rounded corners, fill=white, text width=5cm, font=\footnotesize, align=center]
    at (0,-0.5) {\textit{``Six lectures, one toolkit. Now go build something.''\\[2pt] --- Your Methods \& Algorithms course}};
\end{tikzpicture}
\end{center}

\bottomnote{Course complete! Apply these methods in your group assignment and capstone project.}
\end{frame}
