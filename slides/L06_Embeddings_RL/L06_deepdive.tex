...
%% Slide 5: Word Embeddings: The Representation Problem
\begin{frame}[t]{Why Can't One-Hot Vectors Capture Meaning?}
...
\item Similar words $\rightarrow$ similar vectors (via cosine similarity---defined later)
\item Learn from context (distributional hypothesis)
\end{itemize}
\bottomnote{``You shall know a word by the company it keeps'' -- Firth, 1957}
\end{frame}

%% Slide 6: Skip-gram Architecture (CHART 08)
...
%% Slide 7: Word2Vec: Skip-gram
\begin{frame}[t]{How Does Skip-Gram Learn Word Meaning?}
...
%% Slide 8: Skip-gram: Computational Challenge (HOSTILE REVIEW FIX C1)
\begin{frame}[t]{Why Is Full Softmax Too Expensive?}
...
\item Reduces $O(|V|)$ to $O(k)$ where $k = 5$--$20$ for small datasets; $2$--$5$ for large corpora
\end{itemize}
\bottomnote{Negative sampling: the key innovation that made Word2Vec practical}
\end{frame}

%% Slide 9: Negative Sampling Illustrated (CHART 10)
...
\begin{frame}[fragile,t]{How Does Negative Sampling Work in Practice?}
\begin{algorithmic}[1]
...
\FOR{each epoch}
  \FOR{each word $w_t$ in corpus}
    \FOR{each context word $w_c$ within window}
      ...
      \FOR{$i = 1, \ldots, k$}
        \STATE Sample $w_n \sim P_n(w) \propto f(w)^{3/4}$ \COMMENT{$f(w)$: word freq.}
...
\begin{frame}[t]{Can Vectors Solve Analogies?}
...
%% Slide 13: Word Analogy Limitations (HOSTILE REVIEW FIX M2)
\begin{frame}[t]{When Do Word Analogies Fail?}
\textbf{Known Issues:}
\begin{itemize}
...
%% Slide 14: Similarity Measures (CHART 02, HOSTILE REVIEW Mi3 already applied)
\begin{frame}[t]{How Do We Measure Closeness in Vector Space?}
...
%% Slide 15: Pre-trained Embeddings (HOSTILE REVIEW Mo1 already applied)
\begin{frame}[t]{Should You Train Your Own Embeddings?}
...
%% Slide 16: Static vs Contextual Embeddings (HOSTILE REVIEW FIX M5)
\begin{frame}[t]{Does Context Change a Word's Meaning?}
...
%% Slide 16b: Polysemy Visualization
\begin{frame}[t]{Can You See Why Static Embeddings Fail?}
\centering
\includegraphics[width=0.65\textwidth]{17_static_vs_contextual_embedding/chart.pdf}
\vspace{2mm}
\begin{itemize}
\item ``Bank'' sits between two clusters --- the model can't decide which meaning
\item Contextual models solve this by producing different vectors per context
\item The dashed arrows show where BERT would place each sense of ``bank''
\end{itemize}
\bottomnote{This visualization uses synthetic coordinates but reflects real embedding space behavior}
\end{frame}

%% Slide 17: Embeddings in Finance
\begin{frame}[t]{How Do Banks Use Word Embeddings?}
\textbf{Applications:}
\begin{itemize}
...
%% Slide 18: Finance Example: Embedding-Based Sentiment (HOSTILE REVIEW FIX C2a)
\begin{frame}[t]{How Would You Classify This Headline?}
...
%% Slide NEW: RL in One Picture
\begin{frame}[t]{What Does the RL Landscape Look Like?}
\centering
\begin{tikzpicture}[scale=0.75]
  % Central loop
  \node[draw=mlpurple, fill=mllavender4, rounded corners, minimum width=2cm, minimum height=0.8cm, font=\small\bfseries] (agent) at (0,1.5) {Agent};
  \node[draw=mlblue, fill=mllavender3, rounded corners, minimum width=2cm, minimum height=0.8cm, font=\small\bfseries] (env) at (0,-0.5) {Environment};
  \draw[-{Stealth}, thick, mlpurple] (agent.east) -- ++(1,0) |- (env.east) node[midway, right, font=\small] {action};
  \draw[-{Stealth}, thick, mlblue] (env.west) -- ++(-1,0) |- (agent.west) node[midway, left, font=\small] {state, reward};
  % Roadmap (right side)
  \node[draw=mlgray, rounded corners, fill=white, text width=5.5cm, font=\small, align=left] at (6,1.5) {
    \textbf{Today's roadmap:}\\[3pt]
    1. Bandits (simplest RL)\\
    2. MDPs \& Bellman equation\\
    3. Q-Learning \& exploration\\
    4. Deep Q-Networks\\
    5. Actor-Critic \& PPO (intuition)
  };
  % Finance context
  \node[draw=mlorange, rounded corners, fill=mlorange!10, text width=5cm, font=\small, align=center] at (6,-0.5) {\textbf{Running example:} training a trading agent};
\end{tikzpicture}
\bottomnote{This roadmap covers the RL section. We progress from the simplest case (bandits) to modern methods (PPO).}
\end{frame}

%% Slide NEW: Multi-Armed Bandits
\begin{frame}[t]{What If There Is Only One State?}
\textbf{The Multi-Armed Bandit Problem}

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\includegraphics[width=\textwidth]{top10_16_bandit_regret/chart.pdf}
\end{column}
\begin{column}{0.48\textwidth}
\begin{compactlist}
  \item $k$ actions (``arms''), each with unknown reward distribution
  \item Goal: maximize total reward over $T$ rounds
  \item \textbf{Regret:} $\text{Regret}_T = T \cdot \mu^* - \sum_{t=1}^T r_t$
  \item Finance: ``Which of $k$ trading strategies to allocate capital to?''
\end{compactlist}
\vspace{3pt}
\begin{block}{Key Insight}
Bandits are RL with \textbf{no state transitions} --- the simplest case.
\end{block}
\end{column}
\end{columns}
\bottomnote{Bandits are the gateway to full RL. Next: add state transitions to get MDPs.}
\end{frame}

%% Slide NEW: Bandits to MDP Progression
\begin{frame}[t]{From Bandits to MDPs: Adding Complexity Step by Step}
\centering
\includegraphics[width=0.65\textwidth]{24_bandit_to_mdp_progression/chart.pdf}
\begin{compactlist}
  \item \textbf{Bandit:} One state, $k$ actions --- just explore vs.\ exploit
  \item \textbf{Contextual bandit:} State matters, but actions don't change the state
  \item \textbf{Full MDP:} Actions cause state transitions --- now the future depends on your choices
\end{compactlist}
\bottomnote{Each level adds complexity. The MDP formalism (next slide) captures the full problem.}
\end{frame}
...
%% Slide 19: The RL Framework
\begin{frame}[t]{What Are the Building Blocks of RL?}
...
%% Slide 20: Markov Decision Process (HOSTILE REVIEW Mo4 already applied)
\begin{frame}[t]{What Makes a Problem Markovian?}
...
%% Slide 21: Policy and Value Functions
\begin{frame}[t]{How Do We Measure a Strategy's Worth?}
...
\end{itemize}
\vspace{0.5em}
\textbf{Value Function} ($R_{t+1}$: reward at step $t{+}1$):
\[
...
%% Slide: Bellman Equation (MVF Protocol)
\begin{frame}[t]{Why Is the Bellman Equation Recursive?}
\textbf{Motivate:} Evaluating a policy requires summing infinite future rewards --- impossible directly.

\vspace{-0.3em}
\begin{columns}[T]
\begin{column}{0.40\textwidth}
\textbf{Visualize:} One-step lookahead:
\begin{center}
\begin{tikzpicture}[scale=0.8]
  \node[circle, draw=mlpurple, fill=mllavender4, minimum size=6mm, font=\small] (s) at (0,2) {$s$};
  \node[circle, draw=mlblue, fill=mllavender3, minimum size=6mm, font=\small] (s1) at (-1,0) {$s'_1$};
  \node[circle, draw=mlblue, fill=mllavender3, minimum size=6mm, font=\small] (s2) at (1,0) {$s'_2$};
  \draw[-{Stealth}, thick] (s) -- (s1) node[midway, left, font=\scriptsize] {$a, r_1$};
  \draw[-{Stealth}, thick] (s) -- (s2) node[midway, right, font=\scriptsize] {$a, r_2$};
  \node[font=\scriptsize, below] at (-1,-0.5) {$+\gamma V(s'_1)$};
  \node[font=\scriptsize, below] at (1,-0.5) {$+\gamma V(s'_2)$};
\end{tikzpicture}
\end{center}
\end{column}
...
\]
\textbf{Trading example} ($\gamma = 0.9$):
\begin{compactlist}
  \item Cash $\xrightarrow{\text{buy}}$ Holding: $r = -0.01$ (commission)
  \item Holding $\xrightarrow{\text{sell}}$ Cash: $r = +0.10$ (profit)
  \item $Q(\text{Cash}, \text{buy}) = -0.01 + 0.9 \times 0.10 = 0.08$
\end{compactlist}
\end{column}
\end{columns}
\bottomnote{Bellman equation: value = immediate reward + discounted future. Foundation of all value-based RL.}
\end{frame}

%% Slide 23: Temporal Difference Learning (HOSTILE REVIEW FIX M3)
\begin{frame}[t]{Can We Learn Without Waiting for the End?}
\begin{columns}[T]
\begin{column}{0.66\textwidth}
\textbf{TD(0) Update Rule} (learning rate $\alpha \in (0,1]$) --- learn from each transition:
\[
...
\includegraphics[width=\textwidth]{top10_17_td_learning_update/chart.pdf}
\end{column}
\end{columns}
...
%% Slide 24: Q-Learning Algorithm (HOSTILE REVIEW FIX M9: Robbins-Monro in bottomnote)
\begin{frame}[t]{How Does Q-Learning Find the Best Action?}
\textbf{Update Rule:}
\[
...
\item Choose action $a$ ($\epsilon$-greedy: random with prob.\ $\epsilon$, best otherwise)
...
%% Slide 25: Q-Learning: Worked Example (HOSTILE REVIEW FIX M8)
\begin{frame}[t]{What Happens in One Q-Learning Update?}
...
%% Slide 26: Q-Learning Algorithm: Pseudocode (INSTRUCTOR GUIDE REQUIREMENT)
...
\includegraphics[width=\textwidth]{top10_14_qvalue_convergence/chart.pdf}
\end{column}
\end{columns}
\bottomnote{Watkins \& Dayan (1992). Q-learning. \textit{Machine Learning}, 8(3-4), 279--292.}
\end{frame}

%% Slide 27: Q-Values Visualization (CHART 04)
...
\includegraphics[width=\textwidth]{top10_15_state_action_heatmap/chart.pdf}
\end{column}
\end{columns}
\bottomnote{Left: arrows show policy; colors show Q-values (green=high, red=negative). Right: state-action Q-value heatmap.}
\end{frame}

%% Slide 28: Exploration vs Exploitation
\begin{frame}[t]{Explore or Exploit: How Do You Choose?}
\textbf{The Dilemma:}
\begin{itemize}
...
%% Slide 29: Epsilon Decay Schedule (CHART 12)
...
\begin{itemize}
  ...
  \item \textbf{Exponential:} $\epsilon_t = \epsilon_0 \cdot \beta^t$
...
\end{column}
\end{columns}
\bottomnote{Exponential decay ($\beta = 0.995$) is the most common default; linear is simpler to tune}
\end{frame}

%% Slide 30: RL for Trading
\begin{frame}[t]{Can an Algorithm Learn to Trade?}
\textbf{Formulation:}
\begin{itemize}
...
%% Slide 31: Trading Reward Function Design (HOSTILE REVIEW FIX C2b)
\begin{frame}[t]{How Do You Design the Right Reward Signal?}
...
%% Slide 31b: Reward Shaping Impact
\begin{frame}[t]{How Much Does Reward Design Affect Learning?}
\centering
\includegraphics[width=0.65\textwidth]{19_reward_shaping_trading/chart.pdf}
\vspace{2mm}
\begin{itemize}
\item Same trading environment, two different reward functions
\item Raw P\&L reward (red): high variance, slow convergence
\item Risk-adjusted Sharpe reward (green): smoother, faster learning
\end{itemize}
\bottomnote{Always compare at least 2--3 reward functions before committing to a design}
\end{frame}

%% Slide 32: Backtesting RL Trading Strategies
\begin{frame}[t]{How Do You Know Your RL Strategy Works?}
...
%% Slide: Validation and Policy (Merged)
\begin{frame}[t]{How Do You Validate and Deploy a Trading Policy?}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
...
\textbf{Walk-Forward Validation:}
\begin{compactlist}
  \item Train on $[t_0, t_1]$, test on $[t_1, t_2]$
  \item Roll forward and repeat
  \item Report average OOS performance
\end{compactlist}
\vspace{3pt}
\textbf{Deployment:}
\begin{compactlist}
  \item Compare to buy-and-hold benchmark
  \item Include 0.1--0.5\% transaction costs
  \item Test across bull, bear, sideways regimes
\end{compactlist}
\end{column}
\end{columns}
\bottomnote{Walk-forward: the gold standard for time-series strategies. If your RL agent beats buy-and-hold after costs, verify carefully.}
\end{frame}
...
%% Slide 35: Why Do We Need Deep Q-Networks?
\begin{frame}[t]{What If the State Space Is Too Large for a Table?}
\textbf{Problem:} Tabular Q-learning stores one value per $(s,a)$ pair
\begin{itemize}
\item Trading states (prices, indicators, position) are \highlight{continuous} --- infinite combinations
\item A table with $10^6$ states $\times$ 3 actions = 3 million entries (most never visited)
\end{itemize}
\vspace{0.3em}
\textbf{Solution: Deep Q-Networks (DQN)}
\begin{itemize}
\item Neural network approximates $Q(s, a; \theta) \approx Q^*(s, a)$
\item Generalizes across similar states (nearby prices $\rightarrow$ similar Q-values)
\end{itemize}
\vspace{0.3em}
\textbf{Loss Function:}
\[
...
\bottomnote{DQN: Mnih et al.\ (2015) achieved human-level play on 29/49 Atari games; loss is mean squared TD error}
\end{frame}

%% Slide 36: How Does DQN Stay Stable?
\begin{frame}[t]{How Does DQN Stay Stable?}
\textbf{Two Key Innovations:}
\vspace{0.3em}

\textbf{1. Experience Replay}
\begin{itemize}
\item Store transitions $(s,a,r,s')$ in a buffer; sample random mini-batches
\item Breaks temporal correlation $\rightarrow$ approximately i.i.d.\ training data
\end{itemize}
\vspace{0.3em}
\textbf{2. Target Network} $\theta^-$
\begin{itemize}
\item Separate, slowly-updated copy of the network for computing targets
\item Prevents moving target problem: targets don't shift every gradient step
\end{itemize}
\vspace{-0.3em}
\begin{center}
\includegraphics[width=0.45\textwidth]{09_dqn_architecture/chart.pdf}
\end{center}
\vspace{-0.5em}
\bottomnote{Experience replay + target network: the two innovations that made deep RL stable}
\end{frame}

%% Slide NEW: RL Taxonomy + Actor-Critic/PPO Intuition
\begin{frame}[t]{Where Does Each RL Method Fit?}
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\includegraphics[width=\textwidth]{20_rl_taxonomy_tree/chart.pdf}
\end{column}
\begin{column}{0.50\textwidth}
\begin{compactlist}
  \item \textbf{Value-based:} Learn $Q$-values, pick best action (Q-learning, DQN) --- \textit{this lecture}
  \item \textbf{Policy-based:} Learn actions directly via gradients (REINFORCE, PPO)
  \item \textbf{Actor-Critic:} Actor picks actions, Critic evaluates --- best of both worlds
  \item \textbf{PPO:} Clips policy updates to prevent catastrophic strategy changes
\end{compactlist}
\vspace{3pt}
\begin{block}{For the Math}
See appendix A9 for policy gradient. See L06\_rl\_full for full PPO derivation.
\end{block}
\end{column}
\end{columns}
\bottomnote{Orange = covered in this course. Actor-Critic combines value estimation with direct policy learning.}
\end{frame}
...
%% Slide: Practice (Merged Hands-on + Decision Framework)
\begin{frame}[t]{How Do You Practice and Choose the Right Method?}
\textbf{Hands-on Exercises} (see Colab notebook):
\begin{enumerate}
  \item Explore word embeddings with Word2Vec --- find similar words and visualize clusters
  \item Implement Q-learning on a gridworld and analyze the learned policy
  \item Apply RL to a simple trading environment
\end{enumerate}
\vspace{0.3em}
\textbf{Decision Rules:}
\begin{compactlist}
  \item Text $\rightarrow$ embeddings; sequential decisions $\rightarrow$ RL
  \item Evaluate embeddings via downstream task F1, not just analogy accuracy
  \item RL: always compare against buy-and-hold after realistic transaction costs
\end{compactlist}
\bottomnote{Both transform complex inputs into learnable representations. Notebook: L06 folder on GitHub.}
\end{frame}
...
%% Slide 40: Closing Thought (XKCD #1838 callback with visual)
...
\begin{tikzpicture}[scale=0.7]
  % Robot stick figure
  \node[rectangle, draw, rounded corners, minimum size=7mm, fill=mllavender4] (head) at (0,2.8) {\footnotesize AI};
  \draw[thick] (0,2.4) -- (0,1.4);
  \draw[thick] (0,2.0) -- (-0.5,1.5);
  \draw[thick] (0,2.0) -- (0.5,1.5);
  \draw[thick] (0,1.4) -- (-0.3,0.6);
  \draw[thick] (0,1.4) -- (0.3,0.6);
  % Toolbelt
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (0,-0.1) {6 Methods};
  \node[draw, rounded corners, fill=mllavender4, font=\tiny] at (0,-0.6) {1 Toolkit};
\end{tikzpicture}
\end{center}
\end{column}
...
\normalsize --- Adapted from XKCD \#1838
\end{column}
\end{columns}
\bottomnote{Callback to XKCD \#1838 by Randall Munroe (CC BY-NC 2.5). Course complete!}
\end{frame}

%% Slide: References (Combined)
\begin{frame}[t]{References}
\scriptsize
\textbf{Embeddings:}
\begin{itemize}
\item Mikolov et al.\ (2013). Distributed representations of words and phrases. \textit{NeurIPS}, 3111--3119.
\item Pennington et al.\ (2014). GloVe: Global vectors for word representation. \textit{EMNLP}, 1532--1543.
\item Devlin et al.\ (2019). BERT: Pre-training of deep bidirectional transformers. \textit{NAACL}, 4171--4186.
\item Araci (2019). FinBERT: Financial sentiment analysis with pre-trained language models. arXiv:1908.10063.
\end{itemize}
\vspace{-0.3em}
\textbf{Reinforcement Learning:}
\begin{itemize}
\item Sutton \& Barto (2018). \textit{Reinforcement Learning: An Introduction} (2nd ed.). MIT Press.
\item Mnih et al.\ (2015). Human-level control through deep RL. \textit{Nature}, 518, 529--533.
\item Watkins \& Dayan (1992). Q-learning. \textit{Machine Learning}, 8(3-4), 279--292.
\item Schulman et al.\ (2017). Proximal policy optimization algorithms. arXiv:1707.06347.
\item Levine et al.\ (2020). Offline RL: Tutorial, review, and perspectives. arXiv:2005.01643.
\item Chen et al.\ (2021). Decision Transformer: Reinforcement learning via sequence modeling. \textit{NeurIPS}.
\item Liu et al.\ (2021). FinRL: Deep RL framework for automated trading. \textit{SSRN}.
\end{itemize}
...
%% Slide A7: SARSA vs Q-Learning (Enhanced with Cliff-Walking Chart)
\begin{frame}[t]{SARSA vs Q-Learning}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\includegraphics[width=\textwidth]{21_sarsa_vs_qlearning_cliff/chart.pdf}
\end{column}
\begin{column}{0.48\textwidth}
...
\]
Uses the \textbf{actual next action} $a'$.
...
\]
Uses the \textbf{greedy maximum}.
\end{column}
\end{columns}
\bottomnote{SARSA learns the safe path (accounts for exploration risk); Q-learning learns the optimal but risky path along the cliff edge.}
\end{frame}

%% Slide A8: Statistical Inference for Embeddings & RL
...
%% Slide A9: Policy Gradient Methods
...
%% Slide A10: References and Further Reading
