...
%% Standalone RL lecture for use when taught separately from Embeddings
%% Contains full PPO math that L06_deepdive intentionally omits
\usetheme{Madrid}
\usepackage{graphicx}
\usepackage{booktabs}
...
\begin{enumerate}
  ...
  \item \textbf{Evaluate} model-free RL methods (Q-learning vs SARSA vs PPO) for trading
  \item \textbf{Critique} RL trading strategies accounting for transaction costs, non-stationarity, and offline constraints
\end{enumerate}
...
\begin{frame}{What If There Is Only One State?}
\textbf{The Multi-Armed Bandit} --- simplest RL problem.

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{compactlist}
  \item $k$ actions (``arms''), each with unknown reward
  \item Goal: maximize cumulative reward
  \item \textbf{Regret:} gap between optimal and actual reward
  \item No state transitions --- just explore vs.\ exploit
\end{compactlist}
\end{column}
\begin{column}{0.48\textwidth}
\begin{block}{Finance Example}
Which of $k$ trading strategies should you allocate capital to? You only learn their quality by trying them.
\end{block}
\end{column}
\end{columns}

\bottomnote{Bandits are RL without state transitions. Add transitions $\rightarrow$ you get MDPs (next slide).}
\end{frame}
...
\begin{compactlist}
  \item With probability $\epsilon$: random action
  \item With probability $1-\epsilon$: greedy action
  \item \textbf{Decaying $\epsilon$}: explore early, exploit later
\end{compactlist}
\end{column}
\end{columns}
...
% --- Slide NEW: Actor-Critic ---
\begin{frame}{Why Combine Value and Policy Methods?}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\includegraphics[width=\textwidth]{22_actor_critic_architecture/chart.pdf}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Actor-Critic Architecture:}
\begin{compactlist}
  \item \textbf{Actor}: learns policy $\pi_\theta(a|s)$ directly
  \item \textbf{Critic}: learns value $V_\phi(s)$ to reduce variance
  \item \textbf{Advantage}: $A(s,a) = Q(s,a) - V(s)$\\``Was this action better or worse than average?''
\end{compactlist}
\vspace{3pt}
\textbf{Finance}: Actor decides trades, Critic evaluates portfolio value.
\end{column}
\end{columns}

\bottomnote{Actor-Critic: the foundation for modern algorithms like PPO, A2C, and SAC.}
\end{frame}

% --- Slide NEW: PPO with Math ---
\begin{frame}{How Does PPO Prevent Catastrophic Updates?}
...
\vspace{3pt}
\begin{compactlist}
  \item If the policy changes too much ($r_t$ far from 1), the objective is \textbf{clipped}
  \item Prevents catastrophic forgetting: no sudden strategy flips
  \item $\epsilon \approx 0.2$ is the standard clipping range
  \item PPO is used by OpenAI (ChatGPT via RLHF), DeepMind, and most practitioners
\end{compactlist}

\bottomnote{PPO = Proximal Policy Optimization (Schulman et al., 2017). The workhorse of modern RL.}
\end{frame}

% --- Slide NEW: Offline RL ---
\begin{frame}{What If You Cannot Explore? Offline RL}
\textbf{Problem:} Standard RL requires live interaction. In finance, exploration is costly.

\vspace{3pt}
\textbf{Conservative Q-Learning (CQL):}
\[
\min_Q \;\alpha\,\mathbb{E}_{s \sim D}\!\left[\log \sum_a \exp Q(s,a) - \mathbb{E}_{a \sim \hat{\pi}_\beta}[Q(s,a)]\right] + \frac{1}{2}\,\mathbb{E}_{(s,a,r,s') \sim D}\!\left[\text{TD error}^2\right]
\]
\begin{compactlist}
  \item First term: penalize Q-values for unseen actions (conservative)
  \item Second term: standard Bellman error on the dataset $D$
  \item Result: learns a policy that stays close to the data
  \item \textbf{Decision Transformer}: treats RL as sequence modeling (no value function needed)
\end{compactlist}

\bottomnote{Offline RL (Levine et al., 2020): learn from historical data without new interaction. Critical for finance.}
\end{frame}

% --- Slide NEW: RL Taxonomy ---
\begin{frame}{Where Does Each Method Fit?}
\centering
\includegraphics[width=0.55\textwidth]{20_rl_taxonomy_tree/chart.pdf}
\vspace{-3mm}
\small
\textbf{Value-based}: Q-learning, DQN \quad \textbf{Policy-based}: PPO \quad \textbf{Actor-Critic}: A2C, SAC
\bottomnote{Orange = covered in this course. Most practitioners start with PPO (policy-based + Actor-Critic).}
\end{frame}
...
\item Schulman, J.\ et al.\ (2017). Proximal policy optimization algorithms. arXiv:1707.06347.
\item Levine, S.\ et al.\ (2020). Offline reinforcement learning: Tutorial, review, and perspectives. arXiv:2005.01643.
\item Chen, L.\ et al.\ (2021). Decision Transformer: Reinforcement learning via sequence modeling. \textit{NeurIPS}.
\item Liu, X.-Y.\ et al.\ (2021). FinRL: Deep reinforcement learning framework for automated stock trading. \textit{SSRN}.
\end{itemize}
\bottomnote{Sutton \& Barto: the definitive RL textbook, free at \url{incompleteideas.net}}
\end{frame}
