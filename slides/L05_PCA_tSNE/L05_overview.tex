...
% SLIDE 9: PCA in One Picture (CHART)
% NOTE: chart 08 intentionally shared with deepdive (visual anchor at both levels)
\begin{frame}[t]{PCA in One Picture: Before and After}
\begin{center}
\includegraphics[width=0.65\textwidth]{08_high_dim_before_after/chart.pdf}
\end{center}
\vspace{-0.3em}
\textbf{Key idea:} PCA compresses many dimensions into a few while preserving the dominant structure.
\bottomnote{This is exactly the ``best camera angle'' idea --- mathematically, we formalize it next}
\end{frame}

% SLIDE 10: Key Equations -- PCA
\begin{frame}[t]{Key Equations: PCA}
\textbf{To find the ``best camera angle,'' we measure how features move together and find the direction of greatest spread.}
\vspace{0.3em}

\textbf{Covariance Matrix} \textit{(measures how each pair of features varies together)} (from mean-centered data $X_c = X - \bar{X}$):
\[
C = \frac{1}{n-1}\, X_c^\top X_c
\]

\textbf{Eigendecomposition} \textit{(finds directions and magnitudes of greatest spread)}:
\[
C\, \mathbf{v}_k = \lambda_k\, \mathbf{v}_k
\]

\textbf{Explained Variance Ratio} \textit{(fraction of total information each direction captures)}:
\[
...
\vspace{0.2em}
\textit{The ``best camera angle'' IS the first eigenvector --- the direction capturing the most variance.}
\bottomnote{PCA reduces to an eigenvalue problem on the covariance matrix}
\end{frame}

% SLIDE 11: Key Equations -- t-SNE
...
\vspace{0.1em}
\textit{The Student's $t$-distribution has heavier tails than the Gaussian, preventing points from crowding in 2D.}
\vspace{0.1em}
...
\vspace{0.2em}
\textbf{Objective}: minimize KL divergence --- a measure of how different two probability distributions are:
\[
\text{KL}(P \| Q)
\]
\vspace{0.1em}
\textit{Perplexity ($\approx 30$) controls how many neighbors each point considers --- like adjusting the zoom.}
\bottomnote{The heavy-tailed $t$-distribution in low-D prevents crowding of moderate neighbors}
\end{frame}

\section{Method}

% SLIDE 12: Scree Plot (CHART)
...
% SLIDE 13: Yield Curve PCA (CHART)
\begin{frame}[t]{Yield Curve PCA: Three Factors Explain 98\%}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\includegraphics[width=\textwidth]{09_yield_curve_factors/chart.pdf}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{Three components capture nearly all variation:}
\begin{itemize}
  \item \highlight{PC1 --- Level:} parallel shift ($\sim$85\%)
  \item \highlight{PC2 --- Slope:} steepening ($\sim$10\%)
  \item \highlight{PC3 --- Curvature:} butterfly twist ($\sim$3\%)
\end{itemize}
\vspace{0.3em}
Together: 98\%+ of daily yield curve movements in just 3 numbers.
\end{column}
\end{columns}
\bottomnote{Level--Slope--Curvature decomposition is the foundation of fixed-income risk management}
\end{frame}

% SLIDE 14: PCA on Swiss Roll (CHART)
...
% SLIDE 15: t-SNE on Swiss Roll (CHART)
...
% SLIDE 16: t-SNE Cluster Preservation (CHART)
...
% SLIDE 18: PCA vs t-SNE Comparison Table
...
% SLIDE 20: Market Regime Detection with t-SNE
...
% SLIDE 21: Decision Framework (CHART)
