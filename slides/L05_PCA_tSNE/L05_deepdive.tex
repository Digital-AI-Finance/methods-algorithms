...
%% Slide 5: PCA Intuition
% NOTE: chart 08 intentionally shared with overview (visual anchor at both levels)
\begin{frame}[t]{PCA Intuition: Finding the Best View}
\begin{center}
\includegraphics[width=0.55\textwidth]{08_high_dim_before_after/chart.pdf}
\end{center}
\vspace{-0.3em}
\begin{itemize}
  \item Imagine 2D data scattered in an ellipse
  \item \textbf{PC1} = the long axis of the ellipse (direction of most spread)
  \item \textbf{PC2} = the short axis (remaining spread, perpendicular to PC1)
  \item PCA generalizes this to \textit{any} number of dimensions
\end{itemize}
\bottomnote{PCA finds the ``best camera angle'' --- the view that shows the most variation in the data.}
\end{frame}

%% Slide 6: Key Terms for PCA
\begin{frame}[t]{Key Terms for PCA}
\begin{itemize}
  \item \textbf{Variance:} How spread out data is along one direction; $\text{Var}(X) = \frac{1}{n-1}\sum(x_i - \bar{x})^2$
  \item \textbf{Covariance matrix:} $p \times p$ matrix; entry $(i,j)$ measures how features $i$ and $j$ move together
  \item \textbf{Eigenvector:} A direction that a matrix only \textit{stretches}, never rotates: $A\mathbf{v} = \lambda\mathbf{v}$
  \item \textbf{Eigenvalue} ($\lambda$): The stretch factor; larger $\lambda$ = more variance along that direction
  \item \textbf{Orthogonal:} Perpendicular directions (zero dot product); PCA components are always orthogonal
  \item \textbf{Mean-centering:} Subtract the mean so data is centered at origin; required so PCA measures \textit{variance}, not distance from origin
\end{itemize}
\bottomnote{If these terms feel unfamiliar, review the linear algebra refresher in the course prerequisites.}
\end{frame}

%% Slide 7: PCA by Hand
\begin{frame}[t]{PCA by Hand: A 2D Mini-Example}
\begin{columns}[T]
\begin{column}{0.52\textwidth}
\textbf{Data:} $(1,2),\; (3,4),\; (5,6),\; (2,3),\; (4,5)$

\vspace{0.15em}
\textbf{1.\ Center:} Mean $= (3, 4)$. Centered: $(-2,-2),\; (0,0),\; (2,2),\; (-1,-1),\; (1,1)$

\vspace{0.15em}
\textbf{2.\ Covariance:}
$C = \begin{pmatrix} 2.5 & 2.5 \\ 2.5 & 2.5 \end{pmatrix}$

\vspace{0.15em}
\textbf{3.\ Eigenvalues:} $\lambda_1 = 5,\; \lambda_2 = 0$

\vspace{0.15em}
\textbf{4.\ Eigenvector:} PC1 $= \tfrac{1}{\sqrt{2}}(1,\,1)$

\vspace{0.15em}
\textbf{Result:} 100\% variance in \textbf{1 dimension}.
\end{column}
\begin{column}{0.45\textwidth}
\includegraphics[width=\textwidth]{10_pca_mini_example/chart.pdf}
\end{column}
\end{columns}
\bottomnote{This toy example has $\lambda_2 = 0$ (perfect correlation). Real data has $\lambda_2 > 0$ but small.}
\end{frame}

%% Slide 8: PCA Variance Maximization
...
%% Slide 9: Covariance Matrix and Eigendecomposition
\begin{frame}[t]{Covariance Matrix and Eigendecomposition}
...
%% Slide 11: Why Eigenvectors? The Optimality Proof
\begin{frame}[t]{Why Eigenvectors? The Optimality Proof}
\textbf{We claimed PC1 is the eigenvector with the largest eigenvalue. But why? Could some other direction be better?}
...
%% Slide 13: Variance Explained and Component Selection
...
%% Slide 14: PCA Algorithm
\begin{frame}[t]{The PCA Algorithm}
\begin{algorithmic}[1]
\REQUIRE Data matrix $X \in \mathbb{R}^{n \times p}$, number of components $k$
\ENSURE Projected data $Z \in \mathbb{R}^{n \times k}$, eigenvalues $\lambda_1, \ldots, \lambda_k$
\STATE \textbf{Center:} $X_c \leftarrow X - \bar{X}$ \hfill \textit{(subtract column means)}
\STATE \textbf{(Optional) Standardize} if features have different scales
\STATE \textbf{SVD:} $X_c = U S V^T$ \hfill \textit{(numerically stable)}
\STATE \textbf{Eigenvalues:} $\lambda_k \leftarrow s_k^2 / (n-1)$
\STATE \textbf{Select:} $W_k \leftarrow [v_1, \ldots, v_k]$ \hfill \textit{(top $k$ right singular vectors)}
\STATE \textbf{Project:} $Z \leftarrow X_c \, W_k$
\RETURN $Z$, $\{\lambda_1, \ldots, \lambda_k\}$, $W_k$
\end{algorithmic}
\vspace{0.3em}
\textbf{Complexity:} $O(\min(n^2 p, n p^2))$ via truncated SVD. For large $n$: randomized SVD gives $O(npk)$.
\bottomnote{scikit-learn uses randomized truncated SVD internally --- never forms $X_c^T X_c$ explicitly.}
\end{frame}

%% Slide 15: Reconstruction
...
%% Slide 16: Reconstruction Error vs Components (CHART)
...
%% Slide 18: Yield Curve PCA Worked Example
...
%% Slide 19: Portfolio Risk Decomposition
...
%% Slide 21: PCA: Misconceptions and Assumptions
\begin{frame}[t]{PCA: Misconceptions and Assumptions}
\textbf{Common Misconceptions:}
\begin{itemize}
  \item ``PCA finds clusters'' --- No, PCA finds variance directions; clusters may not align
  \item ``Discarded components are noise'' --- Low-variance PCs may contain task-relevant signal
  \item ``All features must be on the same scale'' --- Only for correlation-based PCA (\texttt{StandardScaler})
\end{itemize}
...
\bottomnote{Challenge yourself: can you construct a dataset where the lowest-variance PC is the most predictive?}
\end{frame}
...
%% Slide 23: t-SNE Mathematical Formulation
...
\vspace{0.2em}
{\small \textit{Why ``Student's''? William Gosset published under the pseudonym ``Student'' in 1908. With 1 degree of freedom, it equals the Cauchy distribution: tails decay as $1/(1+x^2)$ instead of $e^{-x^2}$.}}
\bottomnote{The crowding problem is fundamental to all high-to-low dimensional embeddings, not just t-SNE.}
\end{frame}

%% Slide 25: KL Divergence and the Gradient
...
%% Slide 27: Perplexity Default (CHART)
...
%% Slide 28: Perplexity High (CHART)
...
%% Slide 29: Perplexity Guidelines
...
%% Slide 30: Cluster Preservation: Original Data (CHART)
...
%% Slide 31: Cluster Preservation: PCA Projection (CHART)
...
%% Slide 33: UMAP Modern Alternative
...
%% Slide 35: Market Regime Detection with t-SNE
...
%% Slide 36: PCA Preprocessing for ML Pipelines
...
%% Slide 37: Dimensionality Reduction Comparison Table
...
%% Slide 38: Implementation: PCA, t-SNE, and UMAP
\begin{frame}[t]{Implementation: PCA, t-SNE, and UMAP}
\textbf{PCA} (\texttt{sklearn.decomposition.PCA}):
\begin{itemize}
  \item \texttt{PCA(n\_components=k)} or \texttt{PCA(n\_components=0.95)} for 95\% variance
...
\begin{itemize}
  ...
  \item Supports \texttt{.transform()} for new data (unlike t-SNE)
\end{itemize}
\bottomnote{Always \texttt{StandardScaler} before PCA when features differ in scale. PCA to 30--50 dims before t-SNE/UMAP.}
\end{frame}
...
%% Slide 43: References (Documentation)
...
% SLIDE A8: Statistical Inference for PCA
...
% SLIDE A9: References and Further Reading
