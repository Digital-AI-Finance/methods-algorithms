...
\end{equation}
\scriptsize\textcolor{mlgray}{Note: gradient of SSE (not MSE). For MSE, divide by $n$: $\nabla_{\text{MSE}} = -\tfrac{2}{n}\mathbf{X}^\top(\mathbf{y} - \mathbf{X}\boldsymbol{\beta})$.}
...
\begin{frame}[t]{Convergence Theory}
  ...
  \begin{equation}
    \alpha^* = \frac{1}{\lambda_{\max}(\mathbf{X}^\top\mathbf{X})}
  \end{equation}

  \bottomnote{For OLS, optimal $\alpha = 1/\lambda_{\max}(\mathbf{X}^\top\mathbf{X})$}
\end{frame}
