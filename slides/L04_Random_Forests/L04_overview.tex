...
\begin{itemize}
  \item \highlight{What is a decision tree?} A flowchart-like model that makes predictions by asking a sequence of yes/no questions about features (e.g., ``Is income > \$50k?'' $\to$ ``Is debt ratio > 0.4?''). Each question splits the data; the final answer is at the leaf.
...
\begin{itemize}
  \item \highlight{Internal node} = test one feature against a threshold
  \item \highlight{Branch} = outcome of the test (yes/no)
  \item \highlight{Leaf} = final prediction (class label or value)
  \item \highlight{Path} from root to leaf = one decision rule
\end{itemize}
\vspace{0.3em}
...
\begin{frame}[t]{Overfitting: The Depth Dilemma}
\begin{center}
\includegraphics[width=0.65\textwidth]{17_bias_variance_depth/chart.pdf}
\end{center}
\bottomnote{Deeper trees reduce training error but increase test error---the fundamental bias-variance tradeoff}
\end{frame}

% Chart 20: DT Depth Curve Fitting
\begin{frame}[t]{How Does Tree Depth Affect Curve Fitting?}
\begin{center}
\includegraphics[width=0.65\textwidth]{20_dt_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Shallow trees underfit; deep trees memorize noise. Finding the right depth is key.}
\end{frame}
...
% T9: Bagging Pipeline Cartoon
\begin{frame}[t]{How Bagging Works: From Data to Decision}
\begin{center}
\begin{tikzpicture}[
  >=Stealth,
  box/.style={draw,rounded corners=3pt,minimum width=1.6cm,minimum height=0.8cm,align=center,font=\small},
  tree/.style={isosceles triangle,draw,shape border rotate=90,minimum width=0.7cm,minimum height=0.9cm,inner sep=0pt},
  arr/.style={->,thick}
]
% Training Data
\node[box,fill=mlblue!20] (data) at (0,0) {Training\\Data};

% Bootstrap samples
\node[box,fill=mlorange!20] (b1) at (3.5,1.5) {Bootstrap\\Sample 1};
\node[box,fill=mlorange!20] (b2) at (3.5,0) {Bootstrap\\Sample 2};
\node[box,fill=mlorange!20] (b3) at (3.5,-1.5) {Bootstrap\\Sample 3};

% Arrows from data to bootstrap
\draw[arr] (data.east) -- (b1.west);
\draw[arr] (data.east) -- (b2.west);
\draw[arr] (data.east) -- (b3.west);

% Trees
\node[tree,fill=mlgreen!30] (t1) at (6.5,1.5) {};
\node[font=\tiny] at (6.5,1.5) {Tree 1};
\node[tree,fill=mlgreen!30] (t2) at (6.5,0) {};
\node[font=\tiny] at (6.5,0) {Tree 2};
\node[tree,fill=mlgreen!30] (t3) at (6.5,-1.5) {};
\node[font=\tiny] at (6.5,-1.5) {Tree 3};

% Arrows from bootstrap to trees
\draw[arr] (b1.east) -- (t1.west);
\draw[arr] (b2.east) -- (t2.west);
\draw[arr] (b3.east) -- (t3.west);

% Majority Vote
\node[box,fill=mlpurple!20,minimum width=1.8cm] (vote) at (9.5,0) {Majority\\Vote};

% Arrows from trees to vote
\draw[arr] (t1.east) -- (vote.west);
\draw[arr] (t2.east) -- (vote.west);
\draw[arr] (t3.east) -- (vote.west);
\end{tikzpicture}
\end{center}
\bottomnote{Bootstrap sampling creates diverse training sets; aggregation reduces variance}
\end{frame}
...
\end{itemize}
\vspace{0.5em}
\textbf{In plain language:} the ensemble variance has two parts -- a \highlight{floor} set by how correlated the trees are ($\rho$), and a \highlight{shrinking part} that vanishes as we add more trees ($B$).
\vspace{0.3em}
...
% Chart 21: RF Depth
\begin{frame}[t]{How Does Random Forest Handle Depth?}
\begin{center}
\includegraphics[width=0.65\textwidth]{21_rf_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Averaging many trees produces smooth predictions even at maximum depth.}
\end{frame}
...
\begin{itemize}
  ...
  \item \highlight{Worked example:} node with 70 fraud, 30 legit: $G = 1 - (0.7^2 + 0.3^2) = 1 - 0.58 = 0.42$
\end{itemize}
...
% Chart 23: PR Curve
\begin{frame}[t]{Precision vs Recall: The Fraud Detection Tradeoff}
\begin{center}
\includegraphics[width=0.65\textwidth]{23_precision_recall_curve/chart.pdf}
\end{center}
\bottomnote{In fraud detection, we often sacrifice precision (more false alarms) to catch more actual fraud (higher recall).}
\end{frame}
...
% Chart 26: SMOTE
\begin{frame}[t]{SMOTE: Creating Synthetic Minority Samples}
\begin{center}
\includegraphics[width=0.65\textwidth]{26_smote_visualization/chart.pdf}
\end{center}
\bottomnote{SMOTE generates new minority examples by interpolating between existing minority points.}
\end{frame}
...
% Chart 25: Bagging vs Boosting
\begin{frame}[t]{Bagging vs Boosting: How Do They Reduce Error?}
\begin{center}
\includegraphics[width=0.65\textwidth]{25_bagging_vs_boosting/chart.pdf}
\end{center}
\bottomnote{Bagging reduces variance through averaging; boosting reduces bias by correcting residual errors.}
\end{frame}
...
\begin{itemize}
  \item \highlight{Weak learner:} a model barely better than random guessing (e.g., a tree stump with just one split)
...
\vspace{0.2em}
\begin{center}
\includegraphics[width=0.30\textwidth]{images/1838_machine_learning.png}
\end{center}
\vspace{0.1em}
\begin{center}
\small The beauty of Random Forests: sometimes the simplest ensemble\\
