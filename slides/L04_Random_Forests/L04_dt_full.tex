...
\begin{compactlist}
\item \footnotesize Left child: 40 apps (10/30) $\to G = 0.375$
\item \footnotesize Right child: 60 apps (50/10) $\to G = 0.278$
\end{compactlist}

\vspace{1mm}
Weighted average:
\[
G_{\text{weighted}} = \frac{40}{100} \times 0.375 + \frac{60}{100} \times 0.278 = \highlight{0.317}
\]

Impurity reduction:
\[
\Delta G = 0.48 - 0.317 = \highlight{0.163}
\]
...
\begin{frame}[t]{Depth Effect: Curve Fitting Visualization}
\begin{center}
\includegraphics[width=0.65\textwidth]{20_dt_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Each additional level of depth allows the tree to fit more complex patterns---and more noise.}
\end{frame}
...
% T11: The Tree That Learned Too Much
\begin{frame}[t]{The Tree That Learned Too Much}
\begin{center}
\begin{tikzpicture}[
  >=Stealth,
  every node/.style={font=\small},
  treenode/.style={draw,rounded corners=3pt,minimum width=2.2cm,minimum height=0.65cm,align=center,font=\small},
  leafnode/.style={draw,rounded corners=3pt,minimum width=1.6cm,minimum height=0.55cm,align=center,font=\small},
  level 1/.style={sibling distance=5.5cm,level distance=1.4cm},
  level 2/.style={sibling distance=3cm,level distance=1.4cm},
  level 3/.style={sibling distance=2.5cm,level distance=1.4cm},
  arr/.style={->,thick}
]
\node[treenode,fill=mlblue!20] {Income $>$ \$50k?}
  child {node[treenode,fill=mlblue!20] {Debt ratio $>$ 0.4?}
    child {node[leafnode,fill=mlgreen!20] {Approve}
      edge from parent node[left,font=\tiny] {No}
    }
    child {node[treenode,fill=mlorange!30] {Is it a Tuesday?}
      child {node[treenode,fill=mlred!20] {Did applicant\\blink twice?}
        child {node[leafnode,fill=mlred!40,font=\small\bfseries] {CONFUSED ?!}
          edge from parent node[left,font=\tiny] {No}
        }
        child {node[leafnode,fill=mlgreen!20] {Approve}
      ...
      child {node[leafnode,fill=mlred!30] {Deny}
...
\end{tikzpicture}
\end{center}
\bottomnote{Overfitting: when the model memorizes noise instead of learning patterns}
\end{frame}
