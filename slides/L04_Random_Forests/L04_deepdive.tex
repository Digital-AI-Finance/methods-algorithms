...
\begin{frame}[t]{Regression Trees: Split Visualization}
\begin{center}
\includegraphics[width=0.65\textwidth]{16_regression_tree_mse/chart.pdf}
\end{center}
\bottomnote{A depth-1 regression tree finds the optimal split point that minimizes weighted MSE in child nodes}
\end{frame}
...
% Cost-Complexity Pruning (chart)
\begin{frame}[t]{Cost-Complexity Pruning: Visualization}
\begin{center}
\includegraphics[width=0.65\textwidth]{19_pruning_ccp/chart.pdf}
\end{center}
\bottomnote{Cross-validation identifies the optimal $\alpha$ that balances tree complexity and predictive accuracy}
\end{frame}

% Chart 20: DT Depth Curve Fitting
\begin{frame}[t]{Decision Tree Depth: From Underfitting to Overfitting}
\begin{center}
\includegraphics[width=0.65\textwidth]{20_dt_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Increasing depth trades bias for variance; pruning or ensemble methods control this tradeoff.}
\end{frame}
...
% Chart 22: Correlation and Ensemble Size
\begin{frame}[t]{How Correlation and Ensemble Size Affect Variance}
\begin{center}
\includegraphics[width=0.65\textwidth]{22_correlation_variance_surface/chart.pdf}
\end{center}
\bottomnote{Low tree correlation (via feature randomization) matters more than simply adding more trees.}
\end{frame}
...
% Chart 27: Number of Trees Convergence
\begin{frame}[t]{How Many Trees Are Enough?}
\begin{center}
\includegraphics[width=0.65\textwidth]{27_rf_ntrees_convergence/chart.pdf}
\end{center}
\bottomnote{OOB and test error converge after 100--200 trees; adding more provides diminishing returns.}
\end{frame}
...
% RF max_features OOB Error (chart)
\begin{frame}[t]{Hyperparameters: max\_features Effect}
\begin{center}
\includegraphics[width=0.65\textwidth]{18_rf_hyperparameter_maxfeatures/chart.pdf}
\end{center}
\bottomnote{OOB error as a function of max\_features; the dashed line marks $\sqrt{p}$, the typical default}
\end{frame}
...
% Chart 24: SHAP Waterfall
\begin{frame}[t]{SHAP Values: Explaining a Single Fraud Prediction}
\begin{center}
\includegraphics[width=0.65\textwidth]{24_shap_waterfall/chart.pdf}
\end{center}
\bottomnote{Each feature pushes the prediction higher (toward fraud) or lower (toward legitimate).}
\end{frame}
...
% Bias-Variance Depth Curve (chart)
\begin{frame}[t]{Bias-Variance Tradeoff: Visualization}
\begin{center}
\includegraphics[width=0.65\textwidth]{17_bias_variance_depth/chart.pdf}
\end{center}
\bottomnote{Training error decreases monotonically with depth; test error exhibits the classic U-shaped curve}
\end{frame}
...
% Chart 21: RF Depth Curve Fitting
\begin{frame}[t]{Random Forest Depth: Variance Tamed by Averaging}
\begin{center}
\includegraphics[width=0.65\textwidth]{21_rf_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Unlike a single DT, the random forest produces smooth fits even at unlimited depth.}
\end{frame}
...
% Chart 25: Bagging vs Boosting Convergence
\begin{frame}[t]{Bagging vs Boosting: Convergence Comparison}
\begin{center}
\includegraphics[width=0.65\textwidth]{25_bagging_vs_boosting/chart.pdf}
\end{center}
\bottomnote{Random forests converge smoothly; gradient boosting can overfit if pushed too far.}
\end{frame}
...
% AdaBoost Staged Error (chart)
\begin{frame}[t]{AdaBoost: Training vs Test Error}
\begin{center}
\includegraphics[width=0.65\textwidth]{13_adaboost_staged_error/chart.pdf}
\end{center}
\bottomnote{AdaBoost reduces training error monotonically; test error plateaus as the ensemble converges}
\end{frame}
...
% Gradient Boosting Residuals (chart)
\begin{frame}[t]{Gradient Boosting: First Residual Fit}
\begin{center}
\includegraphics[width=0.65\textwidth]{14_gradient_boosting_residuals/chart.pdf}
\end{center}
\bottomnote{The first tree fits the residuals (data minus mean), moving predictions toward the true function}
\end{frame}
...
% Boosting Learning Rate (chart)
\begin{frame}[t]{Gradient Boosting: Effect of Learning Rate}
\begin{center}
\includegraphics[width=0.65\textwidth]{15_boosting_learning_rate/chart.pdf}
\end{center}
\bottomnote{Smaller learning rates require more iterations but achieve lower test error---the regularization effect}
\end{frame}

% T10: Gradient Descent in Function Space
\begin{frame}[t]{Gradient Boosting: Walking Downhill in Loss Space}
\begin{center}
\begin{tikzpicture}[>=Stealth,scale=0.9]
% Loss curve
\draw[thick,mlpurple] plot[smooth,tension=0.6] coordinates {
  (0,4.5) (1,3.8) (2,2.5) (3,1.8) (4,1.2) (5,0.9) (6,0.7) (7,0.8) (8,1.1) (9,1.8) (10,3.0)
};
\node[font=\small,mlpurple] at (10.5,3.0) {Loss};

% Axes
\draw[->,thick] (-0.3,0) -- (10.5,0) node[right,font=\small] {Function Space};
\draw[->,thick] (0,-0.3) -- (0,5) node[above,font=\small] {Loss};

% Ball positions (decreasing step sizes)
\fill[mlblue] (1.5,3.3) circle (4pt);
\fill[mlorange] (3.5,1.5) circle (4pt);
\fill[mlgreen] (5.0,0.85) circle (4pt);

% Step arrows with decreasing size
\draw[->,very thick,mlblue] (1.6,3.1) -- (3.3,1.7);
\draw[->,thick,mlorange] (3.6,1.35) -- (4.8,0.95);
\draw[->,mlgreen] (5.1,0.8) -- (5.8,0.72);

% Labels
\node[font=\small\bfseries,mlblue,above] at (1.5,3.5) {Tree 1};
\node[font=\small\bfseries,mlorange,below right] at (3.5,1.5) {Tree 2};
\node[font=\small\bfseries,mlgreen,above right] at (5.0,0.85) {Tree 3};

% Step size annotation
\draw[decorate,decoration={brace,amplitude=5pt,mirror},thick,mlgray] (1.5,-0.4) -- (3.5,-0.4);
\node[font=\tiny,mlgray,below] at (2.5,-0.7) {Large step};
\draw[decorate,decoration={brace,amplitude=3pt,mirror},thick,mlgray] (3.5,-0.4) -- (5.0,-0.4);
\node[font=\tiny,mlgray,below] at (4.25,-0.6) {Small step};

% Minimum marker
\draw[dashed,mlgray] (6,0) -- (6,0.7);
\node[font=\tiny,mlgray] at (6,-0.3) {Minimum};
\end{tikzpicture}
\end{center}
\bottomnote{Each tree takes one step in the direction of steepest descent, with the learning rate controlling step size}
\end{frame}
...
% Chart 23: Precision-Recall Curve
\begin{frame}[t]{Precision-Recall Curve: Evaluating Imbalanced Classifiers}
\begin{center}
\includegraphics[width=0.65\textwidth]{23_precision_recall_curve/chart.pdf}
\end{center}
\bottomnote{The PR curve is preferred over ROC when the positive class is rare, as in fraud detection.}
\end{frame}
...
% Chart 26: SMOTE Visualization
\begin{frame}[t]{Visualizing SMOTE Resampling}
\begin{center}
\includegraphics[width=0.65\textwidth]{26_smote_visualization/chart.pdf}
\end{center}
\bottomnote{SMOTE creates synthetic points along line segments between existing minority class neighbors.}
\end{frame}
