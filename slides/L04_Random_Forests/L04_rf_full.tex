...
% Chart 22: Correlation vs Ensemble Size
\begin{frame}[t]{Correlation vs Ensemble Size: The Variance Surface}
\begin{center}
\includegraphics[width=0.65\textwidth]{22_correlation_variance_surface/chart.pdf}
\end{center}
\bottomnote{Decorrelating trees (low $\rho$) through feature randomization is the key to variance reduction.}
\end{frame}

% RF max_features OOB Error (chart)
\begin{frame}[t]{Feature Randomization: OOB Error Curve}
\begin{center}
\includegraphics[width=0.65\textwidth]{18_rf_hyperparameter_maxfeatures/chart.pdf}
\end{center}
\bottomnote{OOB error as a function of max\_features; dashed line marks $\sqrt{p}$, the typical default}
\end{frame}
...
\begin{frame}[t]{Random Forest Depth: Smooth at Every Level}
\begin{center}
\includegraphics[width=0.65\textwidth]{21_rf_depth_curve_fitting/chart.pdf}
\end{center}
\bottomnote{Averaging 200 trees produces stable, smooth predictions regardless of individual tree depth.}
\end{frame}
...
\begin{frame}[t]{Convergence: How Many Trees Do We Need?}
\begin{center}
\includegraphics[width=0.65\textwidth]{27_rf_ntrees_convergence/chart.pdf}
\end{center}
\bottomnote{OOB error provides a free validation metric; test error confirms generalization.}
\end{frame}
...
\begin{frame}[t]{SHAP Waterfall: Individual Prediction Explanation}
\begin{center}
\includegraphics[width=0.65\textwidth]{24_shap_waterfall/chart.pdf}
\end{center}
\bottomnote{SHAP values decompose a prediction into per-feature contributions from a base rate.}
\end{frame}
...
\end{center}
\bottomnote{Smaller learning rates need more iterations but achieve lower test error---the regularization effect}
\end{frame}
...
\begin{frame}[t]{Precision-Recall Analysis for Fraud Detection}
\begin{center}
\includegraphics[width=0.65\textwidth]{23_precision_recall_curve/chart.pdf}
\end{center}
\bottomnote{Selecting the optimal threshold depends on the business cost of false positives vs missed fraud.}
\end{frame}
...
% T12: The Forest Parliament
\begin{frame}[t]{The Forest Parliament}
\begin{center}
\begin{tikzpicture}[>=Stealth,scale=0.85,every node/.style={transform shape}]
% Five trees in semicircle
\foreach \i/\color/\xpos/\ypos in {1/mlblue/-4/0, 2/mlgreen/-2/1.5, 3/mlorange/0/2, 4/mlred/2/1.5, 5/mlpurple/4/0} {
  % Tree trunk
  \draw[\color,line width=2pt] (\xpos,\ypos) -- (\xpos,\ypos+0.6);
  % Tree canopy (triangle)
  \fill[\color!40] (\xpos-0.5,\ypos+0.6) -- (\xpos,\ypos+1.8) -- (\xpos+0.5,\ypos+0.6) -- cycle;
  \draw[\color,thick] (\xpos-0.5,\ypos+0.6) -- (\xpos,\ypos+1.8) -- (\xpos+0.5,\ypos+0.6) -- cycle;
}

% Speech bubbles
\node[draw,rounded corners=3pt,fill=mlblue!10,font=\small\bfseries,text=mlblue] at (-4,2.6) {Fraud!};
\node[draw,rounded corners=3pt,fill=mlgreen!10,font=\small\bfseries,text=mlgreen] at (-2,4.1) {Legit!};
\node[draw,rounded corners=3pt,fill=mlorange!10,font=\small\bfseries,text=mlorange] at (0,4.6) {Fraud!};
\node[draw,rounded corners=3pt,fill=mlred!10,font=\small\bfseries,text=mlred] at (2,4.1) {Fraud!};
\node[draw,rounded corners=3pt,fill=mlpurple!10,font=\small\bfseries,text=mlpurple] at (4,2.6) {Legit!};

% Central banner
\node[draw,rounded corners=5pt,fill=mlpurple!15,line width=1.5pt,draw=mlpurple,
      font=\large\bfseries,minimum width=6cm,minimum height=1cm] at (0,-1.5) {Majority Rules: FRAUD (3--2)};
\end{tikzpicture}
\end{center}
\bottomnote{Democracy in the forest: each tree votes independently, and the majority wins}
\end{frame}
...
\end{column}
\end{columns}
\bottomnote{XKCD \#1885 by Randall Munroe (CC BY-NC 2.5) | L05 preview: PCA finds directions of maximum variance.}
\end{frame}
